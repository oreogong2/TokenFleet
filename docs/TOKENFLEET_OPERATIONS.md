...
- 管理员、私有账本和写入端点只允许经认证访问；匿名只读 `/rank`、公开榜与成员
  ...
  写入口，并使用独立的来源 IP 限流桶；
- 不把 Token 用量用于绩效评价。
- 50 人可以通过一个管理员创建的受限批次链接获得使用资格；批次最多
  50 人、最长 90 天且可关闭（默认 24 小时，90 天需显式选择）。成员填写唯一昵称后各自获得 60 分钟单次设备码；
  共享的是批次能力链接，不是个人设备码或长期凭据。
...
- 第三方服务的个人 URL、访问令牌或 secret 不进入 TokenFleet 客户端、服务端或日志。
...
- 新参赛者不创建邮箱、密码或微信身份；管理员创建单批最多 50 人、最长 90 天的
  受限批次，同一社群可创建多个批次；成员自行登记唯一昵称并获得默认 60 分钟的一次性设备码。第二台设备
  由管理员选择同一参赛者重新生成新码；
- 批次满额、关闭、过期或令牌无效统一对外显示“批次不可用”；原始批次令牌只在
  创建响应返回一次，生产数据库只保存哈希。外传时关闭批次，不删除既有参赛者；
...
应用日志不得记录 Authorization、签名、设备 secret、上报正文或任何第三方凭据。
...
- 社群服务故障：客户端继续本地统计并退避重试；其他第三方独立链路不受影响；
