...
## 当前证据（2026-08-13，beta.8 工作树）

- 服务端与部署模板：本地共收集 `171` 项，`155 passed, 16 skipped`；已有成员安全补发回归确认成员数、批次已领取名额和已使用旧码状态均不改变，遗失的旧未用码立即失效且原始码不落库；16 条 PostgreSQL 专属项在未提供 `TEST_POSTGRES_URL` 时按设计跳过。
- PostgreSQL 17：16 条专属项在既有迁移、并发、限流和 50 名独立参赛者容量回归上，覆盖 60 个并发 batch claim 恰好 50 成功、batch claim 与管理员同名并发恰好一胜、批次 RBAC、匿名响应递归白名单，以及榜外本人签名只读公开汇总；本机临时 PostgreSQL 17 容器已验证 `16 / 16`，对应 PR head 仍必须在 GitHub CI 的 PostgreSQL 17 服务上全部通过。
- Web 单元：`51 passed`，包含匿名公榜合同、大整数精确排序、未定价、多币种、管理员价格录入、无登录参赛者、个人/批次接入码、分享深链与二维码、演示水印、移动导航名称、异步路由竞态、退出内存清理和表单防重复提交。
...
- [x] ZCode macOS 随默认开启的实验总开关采集且可关闭；Windows 默认采集并脱离总开关。Mac 无标记的旧 false 统一迁为开启、新版本显式关闭永久保留，以及只读 SQLite `model_usage` 的完成行、Token 口径、显式总量不一致整桶扣留，均已通过本轮全门禁与增量复核。
...
- [ ] 截图编码、剪贴板、下载错误分层和每日卡／节奏卡已有 XCTest 覆盖；本机只有 Command Line Tools、无 XCTest 模块，必须由 PR 的真实 macOS XCTest 和安装环境权限 E2E 完成最终勾选。
...
- [x] 管理员创建的单批最多 50 人、最长 90 天且可关闭（默认 24 小时，90 天需显式选择）；同一社群可创建多个批次，令牌只存哈希，成员明确同意公开后原子创建参赛者、60 分钟设备码和容量计数。
...
- [x] 跨平台单元检查覆盖 Codex 增量/fork、Claude Code 去重、精确分项拒绝、context-window sentinel、安装时固定 HTTPS origin、配置损坏/升级换 origin/凭据 origin 不匹配时失败关闭、HMAC 金向量、服务端记账响应、分片、连接前凭据探测、严格非敏感状态、安静计划任务和无命令行接入码；本机共收集 28 项，`26 passed, 2 skipped`，两项仅限真实 Windows 环境执行。
