...
- 服务端与部署模板：本地共收集 `166` 项，`150 passed, 16 skipped`；已有成员安全补发回归确认成员数、批次已领取名额和已使用旧码状态均不改变，遗失的旧未用码立即失效且原始码不落库；16 条 PostgreSQL smoke（含并发补发只留一个有效码）在未提供 `TEST_POSTGRES_URL` 时按设计跳过。
