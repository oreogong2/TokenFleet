...
- 服务端：`110 passed, 11 skipped`；默认 SQLite 门禁覆盖租户/RBAC、非登录参赛者、1/2/4 台设备、签名、防重放、共享 usage 限速、自然键硬配额、幂等、质量单调覆盖、组织时区默认日期、公开混合时区提示、公开扫描硬上限、极值价格、重登记密钥轮换、价格、留存、成员禁用、DST、JWT 边界、最大 Token、128 字符模型、安全响应头、隐藏 tombstone、数据库异常参数隐藏、旧迁移和公开组织 readiness；11 条 PostgreSQL smoke 在未提供 `TEST_POSTGRES_URL` 时按设计跳过。
- PostgreSQL 17：`11 passed`，覆盖从空库迁移、公开索引、重复/缺失公开组织 readiness、两个真实 backend 并发同一自然键、组织配额并发严格一胜一拒、跨连接组织/设备限速、exact 与较新 fallback 的连接竞争、注册 token 竞争、稳定设备重登记，以及同版本 tombstone 并发优先级；完整三设备极值 API 黑盒也已通过。
- Web 单元：`48 passed`，包含匿名公榜合同、大整数精确排序、未定价、多币种、管理员价格录入、无登录参赛者、一次性接入码、分享深链与二维码、演示水印、移动导航名称、异步路由竞态和表单防重复提交。
...
- 响应式 Web：管理员 7 条路由和匿名 `/rank`、`/rank/p/{id}`、`/join` 深链均可直接刷新；蓝白主题在 1440/820/390px 通过，长昵称、极值、空榜和未定价无页面级横向滚动；键盘焦点、移动导航名称、对比度和 reduced-motion 均通过。1200×1600 分享海报二维码已由 macOS Vision 实际解码为完整 HTTPS 筛选链接。
...
- [x] 管理员为每台设备生成独立、单次、最长 24 小时的接入码；链接泄露不形成自助注册入口。
...
- [x] 跨平台单元检查覆盖 Codex 增量/fork、Claude Code 去重、精确分项拒绝、context-window sentinel、安装时固定 HTTPS origin、配置损坏/升级换 origin/凭据 origin 不匹配时失败关闭、HMAC 金向量、服务端记账响应、分片、连接前凭据探测、严格非敏感状态、安静计划任务和无命令行接入码，共 22 项通过。
...
- [x] 配置了不存在或重复的公开社群时 readiness 返回 503，不会让匿名榜 404 的实例继续接流量。
- [x] 日志不包含 secret、Authorization、签名或上报正文。
- [x] 速率限制、请求大小、持久行配额和过滤前公开扫描硬上限生效。
- [ ] 5–10 人、14 天真实试运行完成，P0/P1 为 0。
