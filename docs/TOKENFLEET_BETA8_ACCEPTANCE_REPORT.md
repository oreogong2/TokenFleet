...
beta.8 候选已经完成 Swift 分享卡与最终页面真实渲染、服务端 SQLite／PostgreSQL、Web、浏览器联调、Windows 跨平台、universal 构建、安装失败自动回滚与真实原地升级。奥哥已完成主要视觉审核，并接受设置通用页的信息层级作为 beta.9 视觉债。远端 `85fa8358c6c6cf77730a5b09bd8535a64860606a` 已通过四项 CI 和本机单屏“重新打开主窗口”实测；但随后真实连接生产社群时又发现，生产公开榜仍返回 beta.7 行结构且尚无 beta.8 签名个人排名路由。当前候选已补客户端向后兼容：旧响应缺少主力工具／模型时仍展示真实公开榜且不伪造维度；单独的个人排名读取失败时只中立提示暂不可用，不把任意网络或服务器错误归因为部署状态。`85fa8358` 的 CI 不覆盖这次兼容修复，仍必须等待新的精确 PR head 四项 CI 全绿。
