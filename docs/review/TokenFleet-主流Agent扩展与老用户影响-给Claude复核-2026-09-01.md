...
对老用户的答案没有变化：旧客户端可以继续使用，不强制升级；想统计新增来源的用户需要升级 Mac 客户端。服务端的 `tool`、`model` 都是自由字符串，本轮没有服务端字段或 migration，也没有修改 `server/`。

产品授权语义已经由奥哥拍板为方案 1：实验总开关继续自动覆盖未来新增来源，不增加再授权弹窗。已经打开该开关的用户升级后会自动扫描新增固定目录，并可能补计最多 180 天历史。该行为必须通过更新说明明确告知，不能静默发布。
...
| 想统计本轮新增来源 | 旧客户端没有采集器 | 是，只升级 Mac 客户端 |
| 升级，但实验总开关关闭 | 新目录不扫描，新来源不进总量 | 非强制；默认仍为关闭 |
| 升级，且此前已打开实验总开关 | 自动扫描新增目录；存在日志时补计最多 180 天 | 会发生，无再授权弹窗 |
...
不需要重置本地 `usage.json`、设置、设备 secret 或 Keychain，也不需要重新入群。旧设置缺少实验字段时仍默认 false；已有 true 会原样保留，这正是方案 1 自动扩展的触发条件。

## 6. 更新说明的强制告知内容

`CHANGELOG.md` 已增加 Unreleased 说明，明确包含以下三点：

1. 升级后会自动扫描的新增产品目录清单；
2. 已打开实验总开关的用户会自动补计最多 180 天历史；
3. 排行榜和个人总量可能因此上涨，这是正常补计或口径校正，不是统计失真。
...
这里仍需区分“客户端上线”和“新来源立即写生产账本”：现有用户可以直接安装并在本地看到新来源，但如果首版同时把这些新桶写入生产 TeamSync，一旦存在 naturalKey 双计或字段误判，v1 旧行不会自动消失。最小风险做法是利用同一批现有用户做一个短观察窗口：客户端正常发出、本地采集全开，新来源的生产同步延后一版或使用 staging；样本确认后再开放生产排行榜。这样没有另招真机的重复成本，也不拿不可逆账本当测试环境。
