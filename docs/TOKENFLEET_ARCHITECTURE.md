...
管理员创建一个容量 1–50 人、有效期 1–2160 小时（90 天）且可关闭的 InvitationBatch；同一组织
可创建多个批次，50 是单批并发与风险边界，不是组织总成员上限。原始批次
...
- 设备签名的本人排名接口返回同一白名单里的本人主力工具、主力模型和聚合费用，
  使本人不在 Top 10 时仍能生成真实海报；它不返回内部成员 ID、设备或私密字段；
