...
正式发布前必须实际打开并重新核对这些官方页面，搜索摘要或缓存片段不作为价格真源。价目变化必须新建目录版本。价格变化不会伪装成一次“只改金额”的用量刷新：旧快照保持原样，下一次真实用量采集成功后才使用新目录，并显示一次性说明；该次采集若同时发现新增或修正记录，Token 变化仍只来自本地用量记录。旧版本遇到未来或未知价格目录时必须保留原快照并停止覆盖，不能静默降级。
