...
核对日期：2026-08-13；价目版本：`public-usd-2026-08-13`。
...
- OpenAI 使用标准处理、标准上下文公开价。GPT-5.4 超过 272K 输入、GPT-5.6 超过 270K 输入的请求可能适用更高费率；本地日汇总不能可靠复原单次请求上下文，因此界面只称“标准价等价估算”，不称真实费用。
...
正式发布前必须重新核对这些官方页面；价目变化需要新建目录版本，不能静默改写已经落盘的历史估算。
