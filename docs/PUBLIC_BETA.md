...
TokenFleet `v0.1.0-beta.4` 采用“公开源码、邀请接入”的 beta 方式。公开仓库让成员可以审查和在
...
50 人共用同一固定客户端 commit 和社群 origin，但必须拥有独立昵称、独立设备身份
和每台设备单独的一次性码。小的视觉与文案问题进入后续版本；凭据泄露、重复计量、
...
行中，也不能多人共用。设备码在成员开始安装、确认能在一小时内完成时再生成。默认
有效 60 分钟、硬上限 24 小时、使用一次即失效。
...
3. 为这台 Mac 生成一个设备码，默认有效期 60 分钟、最长 24 小时；
