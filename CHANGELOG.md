...
- 将采集 helper 与只读 sqlite3 查询调整为 utility 系统优先级，并把 App 冷启动后的首次自动采集延后约 105 秒；用户手动“立即刷新”仍会立即执行。
- Hermes 只查询统计窗口内的会话，避免大型历史数据库每次全表扫描；不改变 Token 口径、上传协议或历史账本。
- 新增本机采集性能日志，记录总耗时、峰值内存及逐来源耗时／文件数／字节数，不记录文件路径或内容；日志超过 2 MiB 后只保留最近 500 行。
- Windows 客户端、本轮默认开启的实验来源与 ZCode 开关语义均保持 beta.11 行为不变。

## 0.1.0-beta.11 - 2026-09-01
...
- Windows 补齐 16 个自动扫描来源、实验来源残缺桶整桶扣留、异常日志失败隔离、本机页写操作令牌与请求级 Host／Origin 防护、排行榜缓存、独占端口绑定及隔离运行时升级任务重注册，并修复 Windows PowerShell 生成令牌和旧代码页 JSON 输出。PowerShell 动态路径和 3 个 Windows 专属用例以本次发布 CI 与首批真机反馈为最终证据。
