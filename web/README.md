...
- 社群参赛者没有邮箱、密码或 Web session。管理员创建单批最多 50 人、最长 90 天
  且可关闭的受限批次（默认 24 小时，90 天需显式选择）；同一社群可创建多个批次，50 不是总成员上限。成员填写唯一昵称
  并明确同意公开后，各自得到 60 分钟单次码。
...
二维码；点击后先悬浮预览，只提供“保存图片”和“关闭”，不把鉴权信息带进图片
或链接。演示页面和演示海报都有醒目、不可去除的假数据标识。
