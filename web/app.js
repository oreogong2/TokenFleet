...
function renderLogin(message = "") {
  app.innerHTML = `<main class="login-shell">
    ...
    <section class="login-panel">
      ...
      <h1>进入社群管理后台</h1>
      <p class="login-copy"><strong>这里是管理员入口，普通成员无需在此操作。</strong>成员请前往 <a href="/install">安装页</a> 或 <a href="/rank">排行榜</a>；成员批次邀请、一次性设备码和成员昵称不能在此使用。管理员请使用管理员账号登录；服务端签发的会话令牌只保存在当前浏览器标签页，关闭标签页后自动清除。</p>
