...
服务使用单 worker，符合当前进程内登录、公开读取、批次领取和设备登记四类限流
的发布边界。通过 schema 校验的批次领取请求在查询批次 token 前消费独立来源 IP
桶，默认每 60 秒 10 次，不与每分钟 60 次的设备登记桶共享计数。
...
- 当前 2 核 4G、PostgreSQL 17、单 worker 方案按 50 人邀请制 Beta 验收；若要扩大到
  多 worker/多主机，必须先为登录、公开读取、批次领取和设备登记四类端点完成
  跨实例共享限流实测。
